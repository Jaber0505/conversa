...
import { LangModalComponent, Lang } from './ui/lang-modal.component';
...
export class SiteHeaderComponent {
  ...
  openLangModal() { this.showLangModal.set(true); }
  closeLangModal() { this.showLangModal.set(false); }

applyLang(l: Lang) {
    if (l === this.current()) {
        this.closeLangModal(); // ferme quand même si l’utilisateur confirme la langue courante
        return;
    }
    this.langSvc.set(l);
    this.current.set(l);
    this.closeLangModal();   // ✅ ferme le modal après sauvegarde
}
